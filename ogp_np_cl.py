...
def decide_head_for_task(self, orig_task_idx, task_ctx_x, task_ctx_y_onehot):
    ...
    if task_ctx_x.size(1) >= self.cfg.MIN_SAMPLES_FOR_PROTOTYPE:
        ...
        for _ in range(self.cfg.NUM_Z_COLLECTIONS_FOR_PROTOTYPE):
            zm, zlv = self.get_task_z_distribution_params(task_ctx_x, task_ctx_y_onehot)
            czms.append(zm.squeeze(0))
            czlvs.append(zlv.squeeze(0))
        ...
        print(f"Task {orig_task_idx+1}: Ctx size {task_ctx_x.size(1)} < min_samples {self.cfg.MIN_SAMPLES_FOR_PROTOTYPE}. Prior-like proto.")
        azm, azlv = torch.zeros(self.z_dim, device=self.device_internal), torch.zeros(self.z_dim, device=self.device_internal)

    if self.cfg.FIXED_HEAD_PER_TASK:
        nhi = len(self.decoders)
        print(f"Task {orig_task_idx+1}: Fixed head. Spawning head {nhi}.")
        ah = self._spawn_new_head(azm, azlv)
    else:
        if not self.decoders:
            print(f"Task {orig_task_idx+1}: No heads. Spawning head 0.")
            ah = self._spawn_new_head(azm, azlv)
        else:
            ...
            for hi in range(len(self.decoders)):
                ...
                if pm is not None and plv is not None:
                    ...
                    if div < min_div:
                        min_div, ah = div, hi
            if ah != -1 and min_div < self.cfg.Z_DIVERGENCE_THRESHOLD:
                print(f"Task {orig_task_idx+1}: Reusing head {ah}. Div: {min_div:.3f} (Th: {self.cfg.Z_DIVERGENCE_THRESHOLD})")
                # Do NOT update prototype distribution when reusing a head
                self.head_task_counts[ah] += 1
            else:
                reason = f"Min div {min_div:.3f} >= th" if ah != -1 else "No suitable head"
                ni = self._spawn_new_head(azm, azlv)
                print(f"Task {orig_task_idx+1}: {reason}. Spawning new dyn head {ni}.")
                ah = ni

    self.task_to_head_map[orig_task_idx] = ah
    return ah
...
if __name__ == '__main__':
    ...
    results_dir_name = f"results_OGP-NP_{config.EXPERIMENT_TYPE}_{mode_str_for_path}_{config.NUM_CL_TASKS_EFFECTIVE}tasks_seed{config.SEED}_{config.EPOCHS_PER_TASK}epochs_{timestamp}"
